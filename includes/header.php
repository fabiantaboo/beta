...
function renderChatHeader($aei, $isCurrentUserAdmin = false, $formattedEmotions = []) {
    ...
    // Build AEI info section
    $aeiInfo .= '<div class="flex items-center space-x-3">';
    $aeiInfo .= '<div class="relative">';
    $aeiInfo .= '<div class="w-10 h-10 bg-gradient-to-br from-ayuni-aqua to-ayuni-blue rounded-full flex items-center justify-center">';
    $aeiInfo .= '<span class="text-lg text-white font-bold">' . strtoupper(substr($aei['name'], 0, 1)) . '</span>';
    $aeiInfo .= '</div>';
    ...
    if ($isCurrentUserAdmin && !empty($formattedEmotions['strong'])) {
        ...
        $aeiInfo .= '<div class="absolute -bottom-1 -right-1 w-5 h-5 bg-white dark:bg-gray-800 rounded-full flex items-center justify-center border-2 border-white dark:border-gray-800">';
        $aeiInfo .= '<i class="fas ' . $emotionIcon . ' text-xs ' . $emotionColor . '"></i>';
        $aeiInfo .= '</div>';
    }
    
    $aeiInfo .= '</div>';
    $aeiInfo .= '<div>';
    $aeiInfo .= '<h1 class="text-lg font-semibold text-gray-900 dark:text-white">' . htmlspecialchars($aei['name']) . '</h1>';
    $aeiInfo .= '<div class="flex items-center space-x-1">';
    $aeiInfo .= '<div class="w-2 h-2 bg-green-500 rounded-full"></div>';
    $aeiInfo .= '<span class="text-xs text-gray-500 dark:text-gray-400">Online</span>';
